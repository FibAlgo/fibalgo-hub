...
type TabType = 'notifications' | 'settings' | 'alerts';
...
export default function NotificationCenter({ isOpen, onClose }: NotificationCenterProps) {
  ...
  const fetchData = useCallback(async (showLoading = true, checkNewNotifications = false) => {
    ...
    try {
      const [prefsRes, notifRes, alertsRes] = await Promise.all([
        fetch('/api/user/notifications/preferences'),
        fetch('/api/user/notifications/history?limit=20'),
        fetch('/api/user/notifications/alerts?status=active')
      ]);
  ...
  const deleteAlert = async (alertId: string) => {
    try {
      await fetch(`/api/user/notifications/alerts?id=${alertId}`, {
        method: 'DELETE'
      });
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            ...
            { id: 'alerts', label: 'Price Alerts', icon: Target }
          ].map(tab => (
            <button
              key={tab.id}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }}>
            <span style={{ color: '#888', fontSize: '0.75rem', fontWeight: 500 }}>Special Categories</span>
          </div>
          <SettingRow
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          />
          <SettingRow
            icon={<TrendingDown size={14} color="#FF6B6B" />}
            label="Sell Signals"
            checked={preferences.signal_sell}
            onChange={() => togglePreference('signal_sell')}
          />
          <SettingRow
            icon={<TrendingDown size={14} color="#FF4444" />}
...
function AlertsTab({ 
  ...
  const getAlertTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'percent_change_up': return '% Change Up';
      case 'percent_change_down': return '% Change Down';
      case 'volume_spike': return 'Volume Spike';
      case 'breakout_up': return 'Breakout Up';
      case 'breakout_down': return 'Breakout Down';
      default: return type;
    }
  };
  ...
  const getAssetTypeIcon = (type: string) => {
    switch (type) {
      case 'crypto': return <Bitcoin size={14} color="#F7931A" />;
      case 'forex': return <DollarSign size={14} color="#4CAF50" />;
      case 'stocks': return <Briefcase size={14} color="#2196F3" />;
      case 'commodities': return <BarChart3 size={14} color="#9C27B0" />;
      case 'indices': return <BarChart3 size={14} color="#FF9800" />;
    ...
    <div>
      ...
      {alerts.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>No active alerts</p>
          <p style={{ fontSize: '0.8rem', opacity: 0.7 }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                <div style={{
                  ...
                  {getAssetTypeIcon(alert.asset_type)}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ 
                    ...
                    <span style={{
                      ...
                      background: alert.alert_type.includes('above') || alert.alert_type.includes('up')
                        ? 'rgba(0,255,136,0.1)'
                        : 'rgba(255,107,107,0.1)',
                      color: alert.alert_type.includes('above') || alert.alert_type.includes('up')
                        ? '#00FF88'
                        : '#FF6B6B',
                      fontSize: '0.7rem',
                      borderRadius: '4px',
