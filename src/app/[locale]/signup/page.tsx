          ...
          const autoSendVerification = async () => {
            ...
            try {
              ...
              if (data?.code === 'ALREADY_VERIFIED' || resp.status === 409) {
                ...
                setResendStatus({ type: 'success', message: t('verificationSent') });
      ...
      <main style={{ minHeight: '100vh', backgroundColor: '#0A0A0F', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '28rem', textAlign: 'center' }}>
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (data?.code === 'ALREADY_VERIFIED' || (resp.status === 409 && data?.error)) {
                  setAlreadyVerified(true);
                  setResendStatus({ type: 'error', message: data?.error || t('alreadyVerified') });
                  return;
                }
                if (!resp.ok) {
                  const retryAfter = Number(data?.retryAfter || 0);
                  ...
                  setResendStatus({ type: 'success', message: data?.message || t('verificationSent') });
                  const next = Date.now() + RESEND_COOLDOWN_SECONDS * 1000;
                  try { localStorage.setItem(resendCooldownKey, String(next)); } catch {}
                  setCooldownLeft(RESEND_COOLDOWN_SECONDS);
                }
              } catch {
                setResendStatus({ type: 'error', message: t('networkError') });
              } finally {
                setResendLoading(false);
              }
            }}
          ...
          >
            ...
            {resendLoading ? t('resendSending') : cooldownLeft > 0 ? t('resendCooldown', { seconds: cooldownLeft }) : t('resendButton')}
          </button>
          )}
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0A0A0F', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem 1rem 3rem' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '28rem' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '2rem' }}>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF', textAlign: 'center', marginBottom: '2rem' }}>{t('title')}</h1>
          ...
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.875rem', display: 'block', marginBottom: '0.5rem' }}>{t('nameLabel')}</label>
              <div style={{ position: 'relative' }}>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('namePlaceholder')} style={inputStyle} required />
              </div>
            </div>

            <div>
              <label style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.875rem', display: 'block', marginBottom: '0.5rem' }}>{t('emailLabel')}</label>
              <div style={{ position: 'relative' }}>
                <Mail style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', width: '20px', height: '20px', color: 'rgba(255,255,255,0.4)' }} />
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t('emailPlaceholder')} style={inputStyle} required />
              </div>
            </div>

            <div>
              <label style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.875rem', display: 'block', marginBottom: '0.5rem' }}>{t('passwordLabel')}</label>
              <div style={{ position: 'relative' }}>
                <Lock style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', width: '20px', height: '20px', color: 'rgba(255,255,255,0.4)' }} />
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" style={{ ...inputStyle, paddingRight: '3rem' }} required />
            ...
            <div>
              <label style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.875rem', display: 'block', marginBottom: '0.5rem' }}>{t('confirmPasswordLabel')}</label>
              <div style={{ position: 'relative' }}>
                <Lock style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', width: '20px', height: '20px', color: 'rgba(255,255,255,0.4)' }} />
                <input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder={t('confirmPlaceholder')} style={inputStyle} required />
              </div>
              {confirmPassword && password !== confirmPassword && (
                <p style={{ color: '#f87171', fontSize: '0.875rem', marginTop: '0.5rem' }}>{t('passwordsMismatch')}</p>
              )}
            </div>

            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem' }}>
              {t('agreeTerms')} <Link href="/terms-of-service" style={{ color: '#00F5FF', textDecoration: 'none' }}>{t('termsLink')}</Link> {tc('and')} <Link href="/privacy-policy" style={{ color: '#00F5FF', textDecoration: 'none' }}>{t('privacyLink')}</Link>.
            </p>

            <button type="submit" disabled={loading} style={{ ...buttonPrimaryStyle, opacity: loading ? 0.5 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
              {loading ? t('creating') : <><span>{t('createButton')}</span><ArrowRight style={{ width: '20px', height: '20px' }} /></>}
            </button>
          </form>
          ...
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)', marginTop: '1.5rem' }}>
            {t('hasAccount')} <Link href="/login" style={{ color: '#00F5FF', textDecoration: 'none' }}>{t('signInLink')}</Link>
          </p>
        </div>
