...
// Time ago helper — moved inside component to access `t`
...
function TerminalPageContent() {
  ...
  // Time ago helper (needs `t` in scope)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', textShadow: '0 0 6px rgba(255,255,255,0.08)' }}>{t('marketData')}</span>
                <button
                  type="button"
                  ...
                  title={marketDataMinimized ? t('expand') : t('minimize')}
                  aria-label={marketDataMinimized ? t('expand') : t('minimize')}
                >
            ...
            <div ref={newsListScrollRef} className="terminal-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '8px 10px', minHeight: 0, background: 'rgba(0,0,0,0.08)' }}>
              {isLoading ? (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '120px', color: 'rgba(255,255,255,0.5)' }}>
                  <div style={{ textAlign: 'center', fontSize: '0.8rem' }}>{t('loading')}</div>
                </div>
              ) : isAnalyzing && !news.some(n => n.aiAnalysis) ? (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '120px', color: 'rgba(255,255,255,0.5)' }}>
                  <div style={{ textAlign: 'center', fontSize: '0.8rem' }}>{t('analyzing')}</div>
                </div>
              ) : (
              <>
              {news.map((item) => {
                ...
                const timeDisplay = hours > 0 ? `${hours}h ${minutes}m` : minutes > 0 ? `${minutes}m` : t('now');
                  ...
                  <div style={{ 
                    ...
                    <div style={{
                      ...
                      <span style={{
                        ...
                        {isBullish ? '▲' : isBearish ? '▼' : '—'}
                        {isBullish ? t('bullish') : isBearish ? t('bearish') : t('neutral')}
                      </span>
                      <div style={{ 
                        display: 'flex', 
                        alignItems: 'center', 
                        ...
                        <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600 }}>{t('conviction')}</span>
                    ...
                    {uniqueAssets.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' }}>
                        <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, marginRight: '4px' }}>{t('assets')}:</span>
      ...
      <div style={{
        ...
        >
          <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', textShadow: '0 0 6px rgba(255,255,255,0.08)' }}>{t('economicCalendar')}</span>
          <button
            type="button"
            ...
            title={calendarMinimized ? t('expand') : t('minimize')}
            aria-label={calendarMinimized ? t('expand') : t('minimize')}
          >
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center' }} onClick={(e) => e.stopPropagation()}>
            <Link href="/terminal/calendar" style={{ color: 'rgba(0,245,255,0.9)', fontSize: '0.8rem', fontWeight: 500, textDecoration: 'none' }}>{t('viewFullCalendar')}</Link>
          </div>
        </div>
        {!calendarMinimized && (
        <div
        ...
        >
          ...
          >
            {displayEvents.length === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', minWidth: '100%', color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>{calendarEvents.length === 0 ? t('loadingEvents') : t('noUpcomingEvents')}</div>
            ) : (
              displayEvents.map((evt) => {
                ...
                >
                  ...
                  <span title={t('localTimezone')} style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', fontWeight: 600 }}>{formatEventLocalTime(evt.date, evt.time)}</span>
                  ...
                  {(evt.previous != null || evt.forecast != null || evt.actual != null || isLive) && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 8px', fontSize: '0.6rem', marginTop: '1px' }}>
                      {evt.previous != null && evt.previous !== '' && (
                        <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('prev')} <span style={{ color: 'rgba(255,255,255,0.85)' }}>{evt.previous}</span></span>
                      )}
                      {evt.forecast != null && evt.forecast !== '' && (
                        <span style={{ color: 'rgba(0,245,255,0.7)' }}>{t('forecast')}: <span style={{ color: '#00F5FF' }}>{evt.forecast}</span></span>
                      )}
                      {(isLive || (evt.actual != null && evt.actual !== '')) && (
                        <span style={{ color: 'rgba(34,197,94,0.8)' }}>
                          {t('actual')} <span style={{ color: evt.actual ? '#22C55E' : 'rgba(255,255,255,0.4)', fontWeight: evt.actual ? 700 : 400 }}>{evt.actual ?? '—'}</span>
                        </span>
                      )}
                    </div>
                  )}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-[#0A0A0F] to-[#12121A] border border-[#00F5FF]/30 rounded-2xl p-8 max-w-md w-full shadow-2xl shadow-[#00F5FF]/10">
            ...
            <h3 className="text-2xl font-bold text-white text-center mb-4">
              {t('claimIndicatorsTitle')}
            </h3>
            
            <p className="text-gray-400 text-center mb-6">
              {t('claimIndicatorsDesc', { plan: userPlan === 'lifetime' ? 'Lifetime' : 'Ultimate', highlight: t('freeAccess') })}
            </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={t('tradingViewPlaceholder')}
              ...
              >
                {isSubmittingTradingView ? t('saving') : dontWantIndicators ? t('continueWithoutIndicators') : t('submitClaimIndicators')}
              </button>
        ...
        >
          ...
          >
            <div style={{
              ...
              <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.8rem' }}>{t('news')}</span>
              <button
                type="button"
                onClick={() => setOpenedNewsPopup(null)}
                style={{
