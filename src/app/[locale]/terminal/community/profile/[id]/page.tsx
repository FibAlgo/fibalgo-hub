  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (error || !profileData) {
        router.push('/terminal/community');
        return;
      }
  ...
  return (
    <div className="profile-container">
      ...
      <header className="profile-header">
        <Link href="/terminal/community" className="back-btn">
          <ChevronLeft size={24} />
        </Link>
        <div className="header-info">
          <h1>{profile.full_name}</h1>
          <span>{profile.posts_count || 0} {t('posts')}</span>
        </div>
        {isOwnProfile && (
      ...
      <div className="profile-info">
        ...
        <div className="meta-info">
          ...
          <span className="meta-item">
            <Calendar size={16} />
            {t('joinedOn', { date: formatDate(profile.created_at) })}
          </span>
        </div>

        <div className="follow-stats">
          <Link href={`/terminal/community/profile/${profile.id}/following`} className="stat">
            <strong>{profile.following_count || 0}</strong>
            <span>{t('followingCount')}</span>
          </Link>
          <Link href={`/terminal/community/profile/${profile.id}/followers`} className="stat">
      ...
      <div className="posts-list">
        {posts.length === 0 ? (
          <div className="empty-state">
            <Users size={48} />
            <h3>{t('noPosts')}</h3>
            <p>{isOwnProfile ? t('shareFirst') : t('userNoPosts')}</p>
          </div>
        ) : (
          posts.map(post => (
          ...
          <div className="edit-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button 
                ...
                disabled={saving}
              >
                {saving ? <Loader2 size={18} className="spin" /> : t('save')}
              </button>
            </div>
