...
const formatEventLocalTime = (date: string, time?: string, loc?: string): string => {
  if (!date || !time) return '—';
  ...
  return utc.toLocaleTimeString(loc || 'en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
};
...
function getEventCardConfig(event: { type?: string; actual?: string | number; forecast?: string | number; previous?: string; description?: string; date?: string }, tr?: (key: string) => string): { labels: [string, string, string]; values: [string, string, string] } {
  const dash = '—';
  const et = event?.type || 'macro';
  const l = (key: string, fallback: string) => tr ? tr(key) : fallback;
  switch (et) {
    case 'earnings':
      return {
        labels: [l('epsActual', 'EPS ACTUAL'), l('epsEst', 'EPS EST.'), l('prevEps', 'PREV. EPS')],
        values: [event.actual != null ? String(event.actual) : dash, event.forecast != null ? String(event.forecast) : dash, event.previous ?? dash]
      };
    case 'ipo':
      return {
        labels: [l('priceRange', 'PRICE RANGE'), l('exchange', 'EXCHANGE'), l('date', 'DATE')],
    ...
    default:
      return {
        labels: [l('actual', 'ACTUAL'), l('forecast', 'FORECAST'), l('previousLabel', 'PREVIOUS')],
        values: [event.actual != null ? String(event.actual) : dash, event.forecast != null ? String(event.forecast) : dash, event.previous ?? dash]
      };
  }
}
...
const formatDateHeader = (dateStr: string, tr?: (key: string) => string, loc?: string) => {
  ...
  if (date.toDateString() === today.toDateString()) {
    return { day: tr ? tr('today') : 'Today', date: date.toLocaleDateString(locale, opts), isToday: true };
  }
  if (date.toDateString() === tomorrow.toDateString()) {
    return { day: tr ? tr('tomorrow') : 'Tomorrow', date: date.toLocaleDateString(locale, opts), isToday: false };
  }
  return {
    day: date.toLocaleDateString(locale, { weekday: 'short' }),
    date: date.toLocaleDateString(locale, opts),
    isToday: false
  };
};
    ...
    <div style={{
        ...
        {hasActual ? (
          <>
            ...
            <span style={{ color: '#fff', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1px' }}>
              {t('justReleased')} • {minutesAgo < 60 ? `${minutesAgo}m` : `${Math.floor(minutesAgo/60)}h ${minutesAgo%60}m`} {t('ago')}
            </span>
          </>
        ) : isOverdue ? (
          <>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', position: 'relative' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {(() => {
            const cardConfig = getEventCardConfig(event, t);
              ...
              return (
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.7rem', marginBottom: '0.35rem', letterSpacing: '1px' }}>{t('actual')}</div>
                  ...
                  <div style={{
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.7rem', marginBottom: '0.35rem', letterSpacing: '1px' }}>{t('forecast')}</div>
                  ...
                  <div style={{
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.7rem', marginBottom: '0.35rem', letterSpacing: '1px' }}>{t('previousLabel')}</div>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem' }}>{t('urgency')}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem' }}>{t('marketImpact')}</span>
              <span style={{
                        ...
                        <div style={{ 
                          ...
                          {isNoTrade ? (
                            <div style={{
                              ...
                              <div>
                                <div style={{ color: '#6B7280', fontSize: '0.75rem', fontWeight: 700 }}>{t('noTrade')}</div>
                                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem' }}>{playbook?.reason || t('noEdgeInScenario')}</div>
                              </div>
                            </div>
                          ) : trades.length > 0 ? (
                                ...
                                >
                                  ...
                                  <div style={{ flex: 1 }}>
                                    <div style={{ 
                                      ...
                                      {trade.trigger || trade.reasoning || `${isLong ? t('buy') : t('sell')} ${trade.asset}`}
                                    </div>
                                  </div>
                                  ...
                                  {trade.confidence && (
                                    <div style={{
                                      ...
                                      <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.55rem' }}>{t('conf')}</div>
                                      <div style={{ 
...
const UpcomingEventCard = ({ event, analysis, hoursUntil, onAssetClick, isPremium = true, hideTimeInfo = false, cardStyles }: { event: any; analysis: any; hoursUntil: number; onAssetClick?: (symbol: string) => void; isPremium?: boolean; hideTimeInfo?: boolean; cardStyles?: any }) => {
  ...
  const tierColors: Record<number | string, { bg: string; border: string; text: string; color: string }> = {
    1: { bg: 'rgba(239,68,68,0.12)', border: '#EF4444', text: t('tier1'), color: '#EF4444' },
    2: { bg: 'rgba(245,158,11,0.12)', border: '#F59E0B', text: t('tier2'), color: '#F59E0B' },
    3: { bg: 'rgba(59,130,246,0.12)', border: '#3B82F6', text: t('tier3'), color: '#3B82F6' },
    'tier1_market_mover': { bg: 'rgba(239,68,68,0.12)', border: '#EF4444', text: t('tier1'), color: '#EF4444' },
    'tier2_significant': { bg: 'rgba(245,158,11,0.12)', border: '#F59E0B', text: t('tier2'), color: '#F59E0B' },
    'tier3_moderate': { bg: 'rgba(59,130,246,0.12)', border: '#3B82F6', text: t('tier3'), color: '#3B82F6' }
  };
  
  // Event type config - for badges only, not border
  const eventTypeConfig: Record<string, { bg: string; color: string; label: string }> = {
    earnings: { bg: 'rgba(34,197,94,0.15)', color: '#22C55E', label: t('earnings') },
    ipo: { bg: 'rgba(139,92,246,0.15)', color: '#8B5CF6', label: t('ipo') },
    crypto: { bg: 'rgba(247,147,26,0.15)', color: '#F7931A', label: t('crypto') },
    macro: { bg: 'rgba(6,182,212,0.15)', color: '#06B6D4', label: t('macro') }
  };
  ...
  const volatilityConfig: Record<string, { color: string; label: string }> = {
    low: { color: '#22C55E', label: t('volLow') },
    moderate: { color: '#F59E0B', label: t('volModerate') },
    high: { color: '#EF4444', label: t('volHigh') },
    extreme: { color: '#DC2626', label: t('volExtreme') }
  };
    ...
    <div style={{
            ...
            return (
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: cardStyles?.dataCardFontSize || '0.55rem', marginBottom: '0.2rem', letterSpacing: '1px' }}>{t('volatility')}</div>
      ...
      <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: `1px solid ${cardColor}30` }}>
        ...
        {summary && (
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              ...
              <span style={{ color: '#00E5FF', fontSize: cardStyles?.agentFontSize || '0.8rem', fontWeight: 700 }}>{t('fibalgoAgent')}</span>
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.65rem' }}>{t('agentReady')}</span>
            </div>
            <p style={{ 
              color: 'rgba(255,255,255,0.85)', 
            ...
            {summary.length > 200 && (
              ...
              >
                {isAgentExpanded ? (<><ChevronUp size={14} /> {t('showLess')}</>) : (<><ChevronDown size={14} /> {t('readMore')}</>)}
              </button>
            )}
          </div>
        )}
        ...
        >
          ...
          {expanded ? t('hideScenariosAndTrades') : t('showScenariosAndTrades')}
        </button>
              ...
              {keyRisks.length > 0 && (
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span style={{ color: '#EF4444', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.5px' }}>{t('keyRisks')}</span>
                  </div>
              ...
              {event.type === 'earnings' && analysis.expectationsAnalysis && (
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span style={{ color: '#22C55E', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.5px' }}>{t('earningsExpectations')}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('epsEstimate')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('whisper')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('revEstimate')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('revWhisper')}</div>
              ...
              {event.type === 'ipo' && analysis.ipoAnalysis && (
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span style={{ color: '#8B5CF6', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.5px' }}>{t('ipoAnalysis')}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('priceRange')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('expected')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('valuation')}</div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.65rem', letterSpacing: '0.5px', marginBottom: '4px' }}>{t('demand')}</div>
              ...
              {analysis.sectorImpact?.sympathyPlays && analysis.sectorImpact.sympathyPlays.length > 0 && (
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span style={{ color: '#3B82F6', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.5px' }}>{t('sympathyPlays')}</span>
                  </div>
...
export default function CalendarPage() {
    ...
    try {
      ...
      if (!forceRefresh) {
        ...
        if (cache?.calendar && isCacheValid(cache.calendar.timestamp)) {
          ...
          setCalendarEmptyMessage(cachedEvents.length === 0 ? t('noEventsInCache') : null);
  ...
  const formatDateLabel = () => selectedDate.toLocaleDateString(locale, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: '#000' }}>
        ...
        <div style={{
          ...
          {(['all', 'high', 'medium'] as const).map((f) => (
            ...
            >
              ...
              {f === 'all' ? t('filterAll') : f === 'high' ? t('filterHigh') : t('filterMedPlus')}
            </button>
          ))}
        </div>
        ...
        <div ref={countryMenuRef} style={{ position: 'relative' }}>
          ...
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ textTransform: 'uppercase' }}>{countryFilter === 'all' ? t('allCountries') : countryFilter}</span>
            </span>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem' }}>
          <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.75rem' }}>
            {(() => {
              ...
              return <>{gmt} · {t('localTimezone')}</>;
            })()}
          </span>
          <span>
            {selectedDayEvents.length} {selectedDayEvents.length !== 1 ? t('eventsCountPlural', { count: selectedDayEvents.length }) : t('eventsCount', { count: selectedDayEvents.length })} for {selectedDate.toLocaleDateString(locale, { month: 'short', day: 'numeric' })}
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colTime')}</div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600 }}></div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colImpact')}</div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colEvent')}</div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colActual')}</div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colForecast')}</div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', fontWeight: 600, textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('colPrevious')}</div>
      </div>
      </div>
      ...
      <div style={{ padding: '0 0 2rem 0' }}>
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem', color: 'rgba(255,255,255,0.5)' }}>
            ...
            <p style={{ fontSize: '1rem' }}>{t('noEventsFound')}</p>
          ...
          sortedDates.map(dateKey => {
            const dateHeader = formatDateHeader(dateKey, t, locale);
              ...
              <div key={dateKey}>
                <div style={{
                  ...
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', marginLeft: 'auto' }}>
                    {dayEvents.length !== 1 ? t('dayEventsPlural', { count: dayEvents.length }) : t('dayEvents', { count: dayEvents.length })}
                  </span>
                </div>
                ...
                {dayEvents.map((event) => (
                  ...
                  >
                    <div style={{
                      ...
                      <span style={{ flexShrink: 0 }}>{formatEventLocalTime(event.date, event.time, locale)}</span>
                      {event.type === 'crypto' && (
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{
                        ...
                        {(() => {
                          ...
                          const badgeText = badge === 'BEAT' ? t('beat') : badge === 'MISS' ? t('miss') : t('inline');
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ 
                      ...
                      {formatEventLocalTime(eventModalData.event.date, eventModalData.event.time, locale)}
                    </div>
                    {eventModalData.apiEvent && (() => {
                      ...
                      const badgeLabel = isBeat ? t('beatLabel') : isMiss ? t('missLabel') : t('inlineLabel');
                      ...
                      return (
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
                          <span style={{ color: actualColor, fontWeight: 600 }}>
                            {t('actualColon')} {actual ?? '—'}
                          </span>
                          <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('forecastColon')} {forecast ?? '—'}</span>
                          <span style={{ color: 'rgba(255,255,255,0.4)' }}>{t('previousColon')} {eventModalData.apiEvent.previous ?? '—'}</span>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('legendHigh')}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <ImpactIndicator importance="medium" />
          <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('legendMedium')}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <ImpactIndicator importance="low" />
          <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('legendLow')}</span>
        </div>
        <div style={{ width: '1px', background: 'rgba(255,255,255,0.1)' }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <span style={{ color: 'rgba(0,245,255,0.9)', fontWeight: 600, fontSize: '0.85rem' }}>{t('legendAgentReady')}</span>
        </div>
      </div>
