...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: getAlternates('/education', locale),
    openGraph: {
      title: t('metaOgTitle'),
      ...
      images: [{ url: 'https://fibalgo.com/opengraph-image', width: 1200, height: 630, alt: t('jsonLdName') }],
    },
    twitter: {
...
export default async function BlogPage() {
  ...
  const tSlug = await getTranslations('educationSlug');
  const locale = await getLocale();
  ...
  return (
    <main style={{ minHeight: '100vh', width: '100%', overflowX: 'hidden', position: 'relative' }}>
      ...
      <BreadcrumbJsonLd items={[
        { name: tCommon('home'), url: getLocalizedUrl('/', locale) },
                ...
                <div className="blog-featured-meta" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <span style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>
                      {featuredPost.author || tSlug('authorFallback')}
                    </span>
                  </div>
                  <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.35)' }}>
                    {new Date(featuredPost.date).toLocaleDateString(locale, { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                  <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.45)' }}>
                    {timeAgo(featuredPost.date, locale)}
                  </span>
