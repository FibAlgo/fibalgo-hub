...
export default function DashboardClient({ user }: DashboardClientProps) {
              ...
              <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.8)', padding: '1rem' }}>
                <div style={{ background: '#0A0A0F', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '2rem', maxWidth: '450px', width: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                    ...
                    <h3 style={{ color: '#FFFFFF', fontSize: '1.25rem', fontWeight: 600, margin: 0 }}>{t('newTicketTitle')}</h3>
                  </div>
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || t('alertUploadFailed'));
      }
    } catch (error) {
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        alert(t('alertErrorPrefix') + (error.error || t('alertFailedCreateTicket')));
        return;
      }
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        alert(t('alertErrorPrefix') + (error.error || t('alertFailedSendMessage')));
        return;
      }
  ...
  const handleCancelSubscription = async () => {
    if (!cancelReason.trim()) {
      alert(t('alertSpecifyCancelReason'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        alert(t('alertErrorPrefix') + (error.error || t('alertFailedSubmitRequest')));
        return;
      }
  ...
  const handleRefundRequest = async () => {
    if (!refundReason.trim()) {
      alert(t('alertSpecifyRefundReason'));
      return;
    }

    if (!selectedInvoiceForRefund) {
      alert(t('alertNoInvoice'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        alert(t('alertErrorPrefix') + (error.error || t('alertFailedSubmitRefund')));
        return;
      }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        setProfileSaveError(error.error || t('alertFailedUpdateProfile'));
        return;
      }
      ...
      setProfileSaveError(t('alertErrorSavingProfile'));
    }
  };
  ...
  const sendVerificationCode = async (type: 'email' | 'password') => {
    ...
    try {
      ...
      if (!response.ok) {
        setVerificationError(data.error || t('alertFailedSendCode'));
        return;
      }
      ...
      setVerificationError(t('alertFailedSendCode'));
    } finally {
  ...
  const handleVerifyCode = async () => {
    if (verificationCode.length !== 6) {
      setVerificationError(t('alertEnter6DigitCode'));
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setVerificationError(data.error || t('alertInvalidCode'));
      }
    } catch (error) {
      setVerificationError(t('alertError'));
    } finally {
      setVerificationLoading(false);
    }
  };
  ...
  const navigation = [
    { name: t('home'), id: 'home', icon: Home, isLink: true, href: '/' },
    { name: 'FibAlgo Hub', id: 'terminal', icon: Zap, isLink: true, href: '/terminal' },
    { name: t('tabDashboard'), id: 'dashboard', icon: LayoutDashboard },
    { name: t('tabSubscription'), id: 'subscription', icon: CreditCard },
    { name: t('tabSupport'), id: 'support', icon: MessageCircle },
    { name: t('tabSettings'), id: 'settings', icon: Settings },
    // Admin Panel - only visible for admin users (uses persisted isAdmin state)
    ...(isAdmin ? [{ name: t('adminPanel'), id: 'admin', icon: Shield, isLink: true, href: '/admin' }] : []),
  ];
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.8)', padding: '1rem' }}>
          <div style={{ background: '#0A0A0F', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '2rem', maxWidth: '450px', width: '100%' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>{t('categoryLabel')}</label>
              <select
              ...
              >
                <option value="general" style={{ background: '#0A0A0B', color: '#FFFFFF' }}>{t('catGeneral')}</option>
                <option value="billing" style={{ background: '#0A0A0B', color: '#FFFFFF' }}>{t('catBilling')}</option>
                <option value="technical" style={{ background: '#0A0A0B', color: '#FFFFFF' }}>{t('catTechnical')}</option>
                <option value="cancellation" style={{ background: '#0A0A0B', color: '#FFFFFF' }}>{t('catCancellation')}</option>
                <option value="other" style={{ background: '#0A0A0B', color: '#FFFFFF' }}>{t('catOther')}</option>
              </select>
            </div>

            {/* Subject */}
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>{t('subjectLabel')}</label>
              <input
                type="text"
            ...
            {/* Message */}
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>{t('messageLabel')}</label>
              <textarea
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>
                {t('attachmentLabel')} <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem' }}>{t('attachmentHint')}</span>
              </label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <label style={{
                  ...
                  {ticketAttachment ? ticketAttachment.name : t('clickToAddImage')}
                  <input
                    type="file"
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              >
                {ticketAttachmentUploading ? t('uploading') : t('submit')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div style={{ background: '#0A0A0F', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '2rem', maxWidth: '450px', width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
              ...
              <h3 style={{ color: '#FFFFFF', fontSize: '1.25rem', fontWeight: 600, margin: 0 }}>
                {verificationType === 'email' ? t('verifyEmailChange') : t('verifyPasswordChange')}
              </h3>
            </div>

            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '1rem', lineHeight: 1.6 }}>
              {t('codeSentTo')} <span style={{ color: '#FFFFFF', fontWeight: 600 }}>{profileEmail}</span>.
            </p>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              >
                {verificationLoading ? <Loader2 style={{ width: '18px', height: '18px', animation: 'spin 1s linear infinite' }} /> : t('verify')}
              </button>
            </div>
      ...
      <div style={{ marginLeft: isMobile ? 0 : '280px', minHeight: '100vh' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', padding: isMobile ? '0 1rem' : '0 1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <h1 style={{ fontSize: '1.125rem', fontWeight: 600, color: '#FFFFFF', margin: 0 }}>
                {navigation.find(n => n.id === activeTab)?.name || t('tabDashboard')}
              </h1>
            </div>
          </div>
        </header>
        ...
        <main style={{ padding: isMobile ? '1rem' : '1.5rem', maxWidth: isMobile ? '100%' : '900px', margin: '0 auto' }}>
            ...
            <>
              ...
              <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF', marginBottom: '0.5rem' }}>
                  {t('welcomeBack', { name: profileName.split(' ')[0] })} 👋
                </h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.25rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                    <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.875rem' }}>{t('activePlan')}</span>
                    <Crown style={{ 
                      width: '20px', 
                      height: '20px', 
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.25rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                    <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.875rem' }}>{t('daysRemaining')}</span>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.5rem' }}>
                <h3 style={{ color: '#FFFFFF', fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', margin: '0 0 1rem 0' }}>{t('subscriptionDetails')}</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('startDate')}</span>
                    <span style={{ color: '#FFFFFF' }} suppressHydrationWarning>{subscriptionData.startDate || '-'}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <span style={{ color: 'rgba(255,255,255,0.5)' }}>{t('endDate')}</span>
          ...
          {activeTab === 'subscription' && (
            <>
              <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF', marginBottom: '0.5rem' }}>{t('subscriptionMgmt')}</h2>
                <p style={{ color: 'rgba(255,255,255,0.5)', margin: 0 }}>{t('subscriptionMgmtDesc')}</p>
              </div>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                  ...
                  <div>
                    <h3 style={{ color: '#FFFFFF', fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>{t('planLabel', { plan: subscriptionData.plan })}</h3>
                    <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.875rem', margin: 0 }}>
                      {subscriptionData.plan === 'Basic' ? t('basicAccessDesc') : subscriptionData.plan === 'Lifetime' ? t('lifetimeAccessDesc') : t('premiumAccessDesc')}
                    </p>
                  </div>
                </div>
                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '0.5rem', padding: '1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                    <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.875rem' }}>{t('validUntil')}</span>
                    <span style={{ color: '#00F5FF', fontWeight: 600 }}>
                      {subscriptionData.daysRemaining === -1 ? t('unlimited') : t('daysCount', { count: subscriptionData.daysRemaining })}
                    </span>
                  </div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', overflow: 'hidden', marginBottom: '1.5rem' }}>
                <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <h3 style={{ color: '#FFFFFF', fontSize: '1rem', fontWeight: 600, margin: 0 }}>{t('billingHistory')}</h3>
                </div>
                        ...
                        <div>
                          ...
                          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', margin: 0 }}>
                            ...
                            {invoice.paymentMethod === 'credit_card' && (
                              <span style={{ marginLeft: '0.5rem', color: '#10b981', fontWeight: 500 }}>{t('creditCardBadge')}</span>
                            )}
                            ...
                            {invoice.paymentMethod === 'invoice' && (
                              <span style={{ marginLeft: '0.5rem', color: '#6b7280', fontWeight: 500 }}>{t('invoiceBadge')}</span>
                            )}
                          </p>
                        </div>
                      </div>
              ...
              {currentUser?.cancellationRequest?.status === 'approved' && (
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.25rem', marginBottom: '1.5rem' }}>
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>{t('currentPlan')}</span>
                      <span style={{ color: '#fbbf24', fontWeight: 600, fontSize: '0.85rem' }}>{subscriptionData.plan}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>{t('accessUntil')}</span>
                      <span style={{ color: '#FFFFFF', fontWeight: 600, fontSize: '0.85rem' }}>{subscriptionData.endDate}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>{t('daysRemaining')}</span>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>{t('afterEndDate')}</span>
                      <span style={{ color: '#60a5fa', fontWeight: 600, fontSize: '0.85rem' }}>{t('basicPlan')}</span>
                    </div>
                  </div>
                  <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem', margin: '0.75rem 0 0 0', lineHeight: 1.5 }}>
                    ℹ️ {t('cancelInfoMessage', { plan: subscriptionData.plan, endDate: subscriptionData.endDate })}
                  </p>
                </div>
              )}
          ...
          {activeTab === 'support' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div>
                  <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF', marginBottom: '0.5rem' }}>{t('supportTitle')}</h2>
                  <p style={{ color: 'rgba(255,255,255,0.5)', margin: 0 }}>{t('supportSubtitle')}</p>
                </div>
                <button
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '1.25rem', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                    ...
                    <span style={{
                      ...
                      {selectedTicket.status === 'open' ? t('statusOpen') :
                       selectedTicket.status === 'in-progress' ? t('statusInProgress') :
                       selectedTicket.status === 'resolved' ? t('statusResolved') : t('statusClosed')}
                    </span>
                  </div>
                        ...
                        <div style={{
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                            <span style={{ color: msg.senderRole === 'user' ? '#00F5FF' : '#fbbf24', fontSize: '0.8rem', fontWeight: 600 }}>
                              {msg.senderRole === 'user' ? t('you') : t('supportTeam')}
                            </span>
                ...
                <>
                  {userTickets.length > 0 ? (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                              ...
                              <span style={{
                                ...
                                {ticket.status === 'open' ? t('statusOpen') :
                                 ...
                                 ticket.status === 'resolved' ? t('statusResolved') : t('statusClosed')}
                              </span>
                            </div>
                            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', margin: 0 }}>
                              {t('ticketMeta', { count: ticket.messages.length, date: formatTicketDate(ticket.updatedAt) })}
                            </p>
                          </div>
                    ...
                    <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'rgba(255,255,255,0.02)', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                      ...
                      <p style={{ color: '#FFFFFF', fontSize: '1.1rem', fontWeight: 600, margin: 0 }}>{t('noTicketsTitle')}</p>
                      <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem', margin: '0.5rem 0 1.5rem 0' }}>{t('noTicketsDesc')}</p>
                      <button
                        onClick={() => setShowNewTicketModal(true)}
                        style={{
                          display: 'inline-flex',
          ...
          {activeTab === 'settings' && (
            <>
              <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FFFFFF', marginBottom: '0.5rem' }}>{t('settingsTitle')}</h2>
                <p style={{ color: 'rgba(255,255,255,0.5)', margin: 0 }}>{t('settingsSubtitle')}</p>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={currentUser?.tradingViewId ? '' : t('tvPlaceholder')}
                  ...
                  <div style={{ 
                    ...
                    <div>
                      ...
                      <p style={{ color: 'rgba(248,113,113,0.8)', fontSize: '0.8rem', margin: 0 }}>
                        {t('emailOauthNote', { provider: oauthProviderName })}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {/* Current Email */}
                  <div>
                    <label style={{ display: 'block', color: 'rgba(255,255,255,0.7)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t('currentEmail')}</label>
                    <input
                      type="email"
                      value={profileEmail}
                      disabled
                  ...
                  {/* New Email */}
                  <div>
                    <label style={{ display: 'block', color: emailFieldError ? '#f87171' : 'rgba(255,255,255,0.7)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t('newEmail')}</label>
                    <input
                      type="email"
                      value={newEmail}
                ...
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <div>
                      ...
                      <p style={{ color: 'rgba(248,113,113,0.8)', fontSize: '0.8rem', margin: 0 }}>
                        {t('passwordOauthNote', { provider: oauthProviderName })}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', margin: '0 0 1.25rem 0' }}>
                  {t('passwordCooldownNote')}
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {/* Current Password */}
                  <div>
                    <label style={{ display: 'block', color: currentPasswordError ? '#f87171' : 'rgba(255,255,255,0.7)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t('currentPassword')}</label>
                  ...
                  {/* New Password */}
                  <div>
                    <label style={{ display: 'block', color: newPasswordError ? '#f87171' : 'rgba(255,255,255,0.7)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t('newPassword')}</label>
                    <div style={{ position: 'relative' }}>
                      <input
                  ...
                  {/* Confirm Password */}
                  <div>
                    <label style={{ display: 'block', color: confirmPasswordError ? '#f87171' : 'rgba(255,255,255,0.7)', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t('confirmNewPassword')}</label>
                    <input
                      type="password"
