...
export default function DashboardClient({ user }: DashboardClientProps) {
  ...
  useEffect(() => {
    ...
    if (tabParam && ['dashboard', 'subscription', 'support', 'settings'].includes(tabParam)) {
  ...
  const isInvoiceRefundEligible = (invoice: { id: string; date: string; status: string; plan: string; amount: string; paymentMethod?: string; polarOrderId?: string; hasPendingRefund?: boolean }) => {
    // Must be paid
    ...
    const isExtension = /extension/i.test(invoice.plan || '');
