      ...
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        >
          ...
          <div style={{ position: 'relative', zIndex: 2, display: 'grid', gap: '14px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 12px', borderRadius: '999px', background: 'rgba(255,255,255,0.08)', width: 'fit-content' }}>
              ...
              <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.85)' }}>Crypto payment</span>
            </div>
            <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.4rem)', margin: 0, letterSpacing: '-0.01em' }}>
              Complete your plan with crypto
            </h1>
            <p style={{ margin: 0, color: 'rgba(255,255,255,0.75)', maxWidth: '640px', lineHeight: 1.6 }}>
              Pay securely with USDT (or other major coins). We confirm fast and unlock access as soon as the transaction is verified.
            </p>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: '1rem' }}>Step 1 — Send USDT (TRC20)</div>
              <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.95rem' }}>Use the wallet below, then continue with steps 2 and 3.</div>
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.85rem' }}>Wallet</div>
            ...
            >
              <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.85rem' }}>Network</div>
              <div style={{ fontWeight: 700, marginTop: '4px' }}>TRC20 only</div>
            </div>
          </div>
        </div>
        ...
        >
          {[
            {
              title: '2. Share TX hash',
              desc: 'Email us the transaction hash so we can verify instantly.',
              icon: <MessageCircle style={{ width: 18, height: 18, color: PRIMARY }} />,
            },
            {
              title: '3. Access activated',
              desc: 'Once confirmed, we unlock your subscription and send confirmation.',
        ...
        >
          <div className="action-row" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.9rem' }}>
              Need help? We usually reply within minutes during trading hours.
            </div>
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: '1rem' }}>Submit proof</div>
                <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.95rem' }}>Add TX hash and a screenshot. Both are required.</div>
              </div>
            </div>

            <label style={{ display: 'grid', gap: '6px', marginBottom: '10px' }}>
              <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.9rem' }}>Transaction hash *</span>
              <input
                type="text"
                value={txHash}
            ...
            <label style={{ display: 'grid', gap: '6px', marginBottom: '10px' }}>
              <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.9rem' }}>Proof screenshot *</span>
              <input
                type="file"
            ...
            <button
              onClick={async () => {
                ...
                setStatus(null);
                if (!txHash.trim() || !file) {
                  setError('TX hash and proof image are required');
                  return;
                }
                setSubmitting(true);
                try {
                  ...
                  if (!res.ok) {
                    ...
                    setStatus('Submitted. We will review and activate soon.');
                    setTxHash('');
                    setFile(null);
                  }
                } catch (err) {
