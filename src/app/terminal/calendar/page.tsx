...
const formatEventTime = (time?: string, eventType?: string, eventDate?: string) => {
  if (!time) return '—';
  if (time.includes('Before') || time.includes('After')) return time;
  
  try {
    // HH:MM format (EST timezone from Forex Factory)
    if (time.match(/^\d{1,2}:\d{2}/) && eventDate) {
      // Parse as EST and convert to local
      const timeParts = time.match(/^(\d{1,2}):(\d{2})/);
      if (timeParts) {
        const formattedTime = `${timeParts[1].padStart(2, '0')}:${timeParts[2]}`;
        const estDate = new Date(`${eventDate}T${formattedTime}:00-05:00`);
        if (!isNaN(estDate.getTime())) {
          return estDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
        }
      }
      return time;
    }
    ...
    if (hours === 0 && minutes === 0 && eventType === 'crypto') return 'TBA';
...
const groupEventsByDate = (events: CalendarEvent[]) => {
  ...
  Object.keys(grouped).forEach(dateKey => {
    grouped[dateKey].sort((a, b) => {
      ...
      // Parse time strings (HH:MM format or descriptive like "Before Market Open")
      const parseTime = (t: string): number => {
        ...
        // Special cases
        if (t.toLowerCase().includes('before market')) return 0; // 00:00
        if (t.toLowerCase().includes('after market')) return 23 * 60 + 59; // 23:59
        return 12 * 60; // Default to noon for unknown formats
      };
...
export default function CalendarPage() {
  ...
  const fetchEventAnalyses = async (eventList: CalendarEvent[]) => {
    ...
    try {
      ...
      const liveEventItems: Array<{ event: any; analysis: any; minutesAgo: number; hasActual?: boolean }> = [];
      ...
      // Helper function to get event datetime (EST timezone for HH:MM format)
      const getEventDateTime = (event: CalendarEvent): Date => {
        if (event.time && event.time.includes('T')) {
          return new Date(event.time);
        } else if (event.time && event.time.match(/^\d{1,2}:\d{2}/)) {
          // Pad single digit hours
          const timeParts = event.time.match(/^(\d{1,2}):(\d{2})/);
          if (timeParts) {
            const formattedTime = `${timeParts[1].padStart(2, '0')}:${timeParts[2]}`;
            // EST timezone (UTC-5) - Forex Factory times are in EST
            return new Date(`${event.date}T${formattedTime}:00-05:00`);
          }
        }
        // No specific time, use noon
          ...
          if (event.actual) {
            ...
            if (cachedAnalysis) {
              ...
              liveEventItems.push({
                ...
                minutesAgo,
                hasActual: false // Still waiting for actual data
              });
            } else {
      ...
      console.log('📊 FINAL RESULTS:', { preEvent: preEventItems.length, liveEvent: liveEventItems.length });
      console.log('✅ Pre-Event items:', preEventItems.map(p => p.event.name));
      console.log('🔴 Live-Event items:', liveEventItems.map(p => p.event.name));
      setEventAnalyses({ preEvent: preEventItems, liveEvent: liveEventItems });
    } catch (error) {
  ...
  const getEventStartTime = (event: CalendarEvent): Date => {
    // Crypto and earnings events: use start of day (no specific time)
    if (event.type === 'crypto' || event.type === 'earnings' || event.type === 'ipo') {
      return new Date(`${event.date}T00:00:00`);
    }
    // Forex Factory events with ISO time
    if (event.time && event.time.includes('T')) {
      return new Date(event.time);
    } 
    // Forex Factory events with HH:MM format (EST timezone)
    else if (event.time && event.time.match(/^\d{1,2}:\d{2}/)) {
  ...
  const getEventEndTime = (event: CalendarEvent): Date => {
    ...
    // Crypto and earnings events: end at end of day
    if (event.type === 'crypto' || event.type === 'earnings' || event.type === 'ipo') {
      return new Date(`${event.date}T23:59:59`);
    }
    // Macro events: Add 90 minutes (1.5 hours) for "ongoing" period
  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', background: '#000' }}>
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* LIVE EVENTS (Ongoing Events) */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {eventAnalyses.liveEvent.length > 0 && (
        <div style={{ padding: '0 1.5rem' }}>
          ...
          {eventAnalyses.liveEvent.map((item, index) => (
            <LiveEventHero 
              key={`live-${index}`}
              event={item.event}
                ...
                {dayEvents.map((event) => (
                  ...
                  >
                    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem', fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                      {formatEventTime(event.time, event.type, event.date)}
                      {event.type === 'crypto' && (
                        <span style={{
