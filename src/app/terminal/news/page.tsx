...
import AIAnalysisCard, { AIAnalysis } from '@/components/news/AIAnalysisCard';
...
function NewsFeedContent() {
  ...
  // Initialize from cache on mount (client only)
  useEffect(() => {
    const cache = getTerminalCache();
    if (cache?.news?.items && isCacheValid(cache.news.timestamp)) {
      setNews(cache.news.items as NewsItem[]);
      setIsLoading(false);
    }
    if (cache?.sentiment?.data && isCacheValid(cache.sentiment.timestamp)) {
  ...
  const fetchSentiment = async (forceRefresh = false) => {
    try {
      ...
      if (!forceRefresh) {
        const cache = getTerminalCache();
        if (cache?.sentiment && isCacheValid(cache.sentiment.timestamp)) {
  ...
  const fetchNews = async (forceRefresh = false) => {
    try {
      ...
      if (data.news && data.news.length > 0) {
        ...
        // Check for new posts if user has scrolled down
        ...
        // Update last news ID reference
        lastNewsIdRef.current = newNews[0]?.newsId || newNews[0]?.id?.toString();
        
        setNews(newNews);
        // Update cache
        setTerminalCache({
          news: { items: newNews, timestamp: Date.now() }
        });
      }
      setIsLoading(false);
    } catch (error) {
  ...
  // Get breaking news - only show news from last 24 hours
  ...
  const filteredNews = news.filter(item => {
    // Sentiment filter
    if (activeFilter !== 'all' && item.analysis?.sentiment !== activeFilter) return false;
  ...
  return (
    ...
    >
      ...
      <style jsx global>{`
        ...
        /* Mobile tab content - only show News & Feed on desktop */
        .mobile-tab-content {
          display: none;
        }
        .mobile-tab-content.for-you-content {
          display: block;
        }
        .mobile-tab-trends {
          display: none;
          ...
          <div className="breaking-news-container no-swipe" style={{
            ...
            <div style={{ maxHeight: '320px', overflowY: 'auto', position: 'relative', zIndex: 1 }}>
              {breakingNews.map((item, index) => {
                ...
                // Determine sentiment from AI analysis or legacy analysis
                const sentiment = item.aiAnalysis?.analysis?.sentiment || item.analysis?.sentiment;
                const isBullish = sentiment && (sentiment === 'bullish' || sentiment === 'strong_bullish' || sentiment === 'lean_bullish');
                const isBearish = sentiment && (sentiment === 'bearish' || sentiment === 'strong_bearish' || sentiment === 'lean_bearish');
        ...
        <div>
          ...
          <div style={{ 
            ...
            <div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', display: 'flex', overflow: 'hidden', marginBottom: '1rem' }}>
              <div style={{ width: `${(stats.bullish / stats.total) * 100}%`, background: '#22C55E' }} />
              <div style={{ width: `${(stats.neutral / stats.total) * 100}%`, background: '#F59E0B' }} />
              <div style={{ width: `${(stats.bearish / stats.total) * 100}%`, background: '#EF4444' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem' }}>
              <span style={{ color: '#22C55E' }}>Bullish {((stats.bullish / stats.total) * 100).toFixed(0)}%</span>
              <span style={{ color: '#F59E0B' }}>Neutral {((stats.neutral / stats.total) * 100).toFixed(0)}%</span>
              <span style={{ color: '#EF4444' }}>Bearish {((stats.bearish / stats.total) * 100).toFixed(0)}%</span>
            </div>
          </div>
        </div>
