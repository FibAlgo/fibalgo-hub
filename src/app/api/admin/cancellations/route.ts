...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: refundedBilling } = await supabaseAdmin
    ...
    if (refundedBilling) {
      await supabaseAdmin
        .from('cancellation_requests')
        .update({ status: 'rejected', admin_note: 'Cancelled due to refund processed' })
        .eq('user_id', userId)
        .eq('status', 'pending');
      return NextResponse.json({ error: 'Refund already processed. Cancellation is not allowed.' }, { status: 400 });
    }
