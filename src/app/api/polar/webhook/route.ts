...
import { PlanType, isSandbox } from '@/lib/polar';
...
// Helper: Get plan type from product name
function getPlanFromProduct(productName: string): PlanType {
...
async function trackTradingViewDowngrade(params: {
  ...
  await supabaseAdmin
    ...
    .insert({
      ...
      tradingview_id: user.trading_view_id,
...
async function trackTradingViewUpgrade(params: {
  ...
  // Only track Ultimate upgrades (they need TradingView access)
  if (newPlan !== 'ultimate') {
  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'subscription.created': {
        ...
        // Track TradingView upgrade if ultimate
        if (plan === 'ultimate') {
      ...
      case 'order.paid': {
        ...
        const metadata = order.metadata as { user_id?: string } | null;
        ...
        await addBillingHistory({
          ...
          description: `${productName} - ${order.billing_reason || 'Purchase'}`,
          invoiceUrl,
          invoiceId,
          billingReason: order.billing_reason,
        });
        
        // Track TradingView upgrade for admin to grant access (only for Ultimate)
        await trackTradingViewUpgrade({
          userId: user.id,
          newPlan: plan,
          polarSubscriptionId: order.subscription_id || undefined,
      ...
      case 'order.refunded': {
        ...
        console.log(`[Polar Webhook] Adding refund to billing history: -$${refundAmount / 100}`);
