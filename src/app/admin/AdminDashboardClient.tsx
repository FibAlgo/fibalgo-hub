...
export default function AdminDashboardClient({ userId }: AdminDashboardClientProps) {
  ...
  const expiredSubscriptions = users.filter(u => {
    if (u.subscription.plan === 'basic') return false;
  ...
  useEffect(() => {
    const loadCancellations = async () => {
      try {
        ...
        // Fallback
        const cancellations = await getPendingCancellations();
  ...
  const handleAddUser = async () => {
    ...
    await createUser(newUserEmail, newUserName, newUserTradingView);
    await loadUsers();
    setNewUserEmail('');
    setNewUserName('');
          ...
          {activeTab === 'settings' && (
            <>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1rem', padding: '1.5rem' }}>
                <p style={{ color: 'rgba(255,255,255,0.5)', textAlign: 'center' }}>Ayarlar yakında eklenecek...</p>
              </div>
            </>
          )}
