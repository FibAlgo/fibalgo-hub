...
export const config = {
  matcher: ['/((?!api|_next/static|_next/image|favicon.ico|robots.txt|sitemap.xml|manifest.webmanifest|sw-notifications.js|images|icon-|apple-touch-icon|opengraph-image|.*\\.png$|.*\\.ico$|.*\\.svg$|.*\\.webp$|.*\\.jpg$|.*\\.jpeg$|.*\\.gif$|.*\\.woff2?$|.*\\.js$|.*\\.html$|.*\\.txt$).*)'],
};
